...
export default class SubscriptionService {
	...
	async get(id: string): Promise<Subscription> {
		...
		const result = await this._connection.query(`
			SELECT
				...
				c.length, c.position AS cycle_position, c.next_position, c.price, c.shipping_price,
	...
	async create(productSchedule: ProductSchedule, customerId: string, merchantProviderId: string): Promise<string> {
		...
		return this._connection.withTransaction(async () => {
			...
			const cycleValues = PostgresConnection.getValuesSubstitution(cycleParams);
			...
			const cycleProductValues = PostgresConnection.getValuesSubstitution(cycleProductParams);
	...
	async update(partialSubscription: Partial<Subscription>): Promise<void> {
		...
		return this._connection.withTransaction(async () => {
			...
			const cycleIds = _.filter(_.map(partialSubscription.cycles, cycle => cycle.id), id => id !== undefined);
			await this._connection.query(`
			...
			// Update the subscription row itself.
			await this._connection.query(`
			...
			// Filter oldCycles before adding ids to newCycles, since that's how we tell them apart!
			...
			if (newCycles.length > 0) {
				...
				const newCycleValues = PostgresConnection.getValuesSubstitution(newCycleParams);
				const cycleResult = await this._connection.query(`
					INSERT INTO subscriptions.subscription_cycle
					(subscription_id, name, length, position, next_position, price, shipping_price) VALUES
					${newCycleValues}
			...
			if (oldCycles.length > 0) {
				...
				const oldCycleValues = PostgresConnection.getValuesSubstitution(oldCycleParams);
				await this._connection.query(`
			...
			const cycleProductIdValues = PostgresConnection.getValuesSubstitution(cycleProductIds);
			await this._connection.query(`
				DELETE FROM subscriptions.subscription_cycle_product scp
				USING (VALUES
