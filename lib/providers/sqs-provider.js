...
module.exports = class SQSProvider extends AWSProvider {
	...
	receiveMessagesRecursive(parameters, count) {

		du.debug('Receive Messages Recursive');

		count = (_.isUndefined(count) || _.isNull(count)) ? 0 : count;

		let return_messages = [];

		if (count >= this.batch_read_limit) {
			return Promise.resolve(return_messages);
		}

		let limit = this.batch_read_limit - count;

		if (limit < 10) {
			parameters.limit = limit;
		}

		return this.receiveMessages(parameters).then((messages) => {

			return_messages = arrayutilities.merge(messages, return_messages);

			count = parseInt(count + parseInt(messages.length));

			if (messages.length == 10 && (count < this.batch_read_limit)) {

				return this.receiveMessagesRecursive(parameters, count).then(messages => {

					return_messages = arrayutilities.merge(return_messages, messages);
