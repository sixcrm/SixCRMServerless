...
class SQSUtilities {
    ...
    getQueueURL(input){
      ...
      if(_.isString(input)){
        ...
        if(_.has(input, 'queue') && _.isString(input.queue)){
          shortname = input.queue;
        }
      }

      if(_.isNull(shortname)){
        eu.throwError('server', 'Unable to determine queue shortname.');
      }
      ...
      return 'https://sqs.'+process.env.aws_region+'.amazonaws.com/'+process.env.aws_account+'/'+shortname;
    ...
    deleteQueue(shortname) {
      ...
      du.debug('Delete Queue', shortname);
