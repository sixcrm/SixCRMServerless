...
const parseDate = pg_types.getTypeParser(1114);

[1114].map((oid) => { // Also at postgres following date fields oids available: 1184(timestamp), 1082(date), 1114(timestamp without timezone)
  pg_types.setTypeParser(oid, v => timestamp.convertToISO8601(parseDate(v)));
});
