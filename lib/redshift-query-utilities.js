...
class RedshiftQueryUtilities {
    ...
    constructor(){
      ...
      if(process.env.TEST_IMAGE == 'true'){
        du.highlight(' TESTING TESTING');
        this.redshift_configuration = global.SixCRM.configuration.site_config.docker_postgresql;
      } else {
        du.highlight(' Not good ' + process.env.TEST_IMAGE);
      ...
      if(errors.length > 0){
        ...
        return global.SixCRM.configuration.getEnvironmentConfig('redshift.host').then((host) => {
          ...
          if(process.env.TEST_IMAGE == 'true'){
            ...
            du.warning('Using docker config file for testing....');
            host = this.redshift_configuration.host;

          }

          if(_.isNull(host) || process.env.TEST_IMAGE==true){
          ...
          du.info('Postgress configuration:', pg_config);
    ...
    query(query, parameters){
      ...
      return new Promise((resolve) => {
        ...
        return this.instantiateRedshift().then((result) => {
