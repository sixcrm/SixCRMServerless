...
class RedshiftQueryUtilities {
    ...
    this.quiting_timer = setTimeout(() => {
      ...
      this.closeConnection()
        .catch((error) => {
          ...
          return Promise.reject(error); // eu.throwError('server', error);
  ...
  query(query, parameters) {
    ...
    let new_query_promise = new Promise((resolve, reject) => {
      ...
      return this.instantiateRedshift()
        ...
        .catch(error => {
          ...
          console.log(this.id, '>>>>>>> Failed Query Redshift', error);

          return reject(error); // eu.throwError('server', error);
