...
module.exports = class RedisUtilities {
    ...
    setDefaultExpiration(){
      ...
      this.default_expiration = this.site_config.elasticache.default_expiration;
    ...
    createClient(){
        ...
        if(!_.has(this.site_config.cache, 'usecache') || (parseInt(this.site_config.cache.usecache) < 1)){
        ...
        let configuration_object = {
          host: this.site_config.elasticache.endpoint,
          port: this.site_config.elasticache.port,
    ...
    prepare(attempts){
        ...
        if(attempts > this.site_config.elasticache.max_attempts){
    ...
    retryStrategy(options){
        ...
        if(
        (_.has(this.site_config.elasticache, 'retry_time') && options.total_retry_time > parseInt(this.site_config.elasticache.retry_time)) ||
        (!_.has(this.site_config.elasticache, 'retry_time') && options.total_retry_time > parseInt(this.connection_defaults.retry_time))
      ){
        ...
        if(
        (_.has(this.site_config.elasticache, 'max_attempts') && options.attempt > parseInt(this.site_config.elasticache.max_attempts)) ||
        (!_.has(this.site_config.elasticache, 'max_attempts') && options.attempt > parseInt(this.connection_defaults.max_attempts))
      ){
