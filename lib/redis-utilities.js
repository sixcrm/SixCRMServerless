...
class RedisUtilities {
  ...
  constructor() {
    ...
    this.quiting_timer_timeout_ms = 2000;
  }
  ...
  executeAndScheduleQuit(promised_callback) {
    ...
    if (!this.redis_client_promisified) {
      ...
      du.deep('redis-itils: connected');
    }
    return queue
      .then(() => {
        du.deep('redis-itils: querying');
      ...
      .then((result) => {
        du.deep('redis-itils: scheduling quit');
  ...
  // TODO: caching example
  // tryCache(unique_cache_id, calc_value_calback, expiration) {
  //   if (!this.redis_client) {
  //     return calc_value_calback();
  //   }
  //   return this.get(unique_cache_id)
  //     .then(value => {
  //       if (value !== undefined) {
  //         return value;
  //       }
  //       const new_value = calc_value_calback();
  //
  //       return this.set(unique_cache_id, new_value, expiration)
  //         .then(() => new_value);
  //     })
  // }
}
