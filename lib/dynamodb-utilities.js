...
class DynamoDBUtilities extends AWSUtilities{
    ...
    constructor(){
      ...
      if(process.env.stage === 'local' || process.env.IS_OFFLINE) {
        ...
        let endpoint = global.SixCRM.configuration.site_config.dynamodb.endpoint;

        if(!_.isUndefined(endpoint) && !_.isNull(endpoint)){

          AWS.config.update({ accessKeyId: "localhost", secretAccessKey: "localhost", region: "us-east-1" });
    ...
    executeDynamoDBMethod({method, parameters, status, fatal}){
      ...
      let valid_methods = {
        ...
        raw: ['waitFor', 'deleteTable', 'describeTable', 'updateTable', 'createTable', 'createBackup']
      }
