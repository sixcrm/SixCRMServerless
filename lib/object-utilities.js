...
module.exports = class ObjectUtilities {
  ...
  // TODO: use this method instead of getRecursive() in the future.
  //Technical Debt:  Test Me!
  // static getByPath(object, path, default_value){
  //
  //   du.deep('Get Recursive');
  //
  //   if(!arrayutilities.isArray(path)){
  //     if(!_.isString(path)){
  //       return eu.throwError('server', 'Path must be an array or a string.');
  //     }
  //
  //     path = path.split('.');
  //
  ...
  //
  //   if(path.length < 1){
  //     return eu.throwError('server', 'path array must be of length 1 or greater.');
  //   }
  //
  //   let current_obj = object;
  //   const return_fail = (fail_msg) => {
  //
  //     if (_.isUndefined(default_value)) {
  //       return eu.throwError('server', fail_msg);
  //     }
  //
  ...
  //
  //     if(!_.isString(key)){
  //       return eu.throwError('server', 'Non-string key observed.');
  //     }
  //
  //     if (!_.isObject(current_obj)){
  //       return return_fail('Can\'t get value by path - not object faced.');
  //     }
  //
  //     if (!_.has(current_obj, key)){
  //       return return_fail('Expected object to have key "'+arrayutilities.compress(key,'.','')+'"');
  //     }
  //
