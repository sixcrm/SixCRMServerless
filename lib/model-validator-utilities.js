...
module.exports = class ModelValidatorUtilities {
    ...
    static loadReferencesRecursive(path_to_model, parse_function, count) {
        ...
        count || (count = 0);
        du.deep(`Load References Recursive, [${count}]`);
        ...
        du.deep(`Looking for references in ${path_to_model}`);
        delete require.cache[require.resolve(path_to_model)];
        let schema = require(path_to_model);
        //du.debug(schema);
        ...
        du.deep(`Found ${references.length} reference(s).`);

        arrayutilities.map(references, (reference) => {

          du.deep(`Found reference to ${reference}`);
    ...
    static validateModel(validation_object, path_to_model, validation_function, fatal){
        ...
        try {
            ...
            eu.throwError('server','One or more validation errors occurred: Object to validate must be an object, got ' + typeof validation_object);
        }
    ...
    */
    ...
    static buildPrettyErrorMessage(validation){
      ...
      arrayutilities.map(validation.errors, validation_error => {
        if(objectutilities.hasRecursive(validation_error, 'parentSchema.title') && _.has(validation_error, 'message')){
          error_messages.push(validation_error.parentSchema.title+' '+validation_error.message);
        }
      })
