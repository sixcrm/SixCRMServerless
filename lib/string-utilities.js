...
module.exports = class StringUtilities {
	...
	static isUUID(a_string, version) {
		...
		if (this.isString(a_string)) {
			...
			try {
				return mvu.validateModel(a_string, global.SixCRM.routes.path('model', 'definitions/uuidv' + version), null, false)
			} catch (e) {
				return false;
			}
		}
	...
	static isURL(a_string) {
		...
		if (this.isString(a_string)) {
			...
			try {
				return mvu.validateModel(a_string, global.SixCRM.routes.path('model', 'definitions/url.json'))
			} catch (e) {
				return false;
			}
	...
	static isPhone(a_string) {
		...
		if (this.isString(a_string)) {
			...
			try {
				return mvu.validateModel(a_string, global.SixCRM.routes.path('model', 'definitions/phone.json'))
			} catch (e) {
				return false;
			}
	...
	static isEmail(a_string) {
		...
		if (this.isString(a_string)) {
			...
			try {
				return mvu.validateModel(a_string, global.SixCRM.routes.path('model', 'definitions/email'), null, false)
			} catch (e) {
				return false;
			}
