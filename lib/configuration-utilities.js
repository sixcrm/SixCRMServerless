...
module.exports = class ConfigurationUtilities{
  ...
  static getEntityConfig(config_path){

    du.debug('Get Config of Entity');

    let config = global.routes.include('deployment', config_path+'/config.json');

    if (!config) {
      eu.throwError('server', 'ConfigurationUtilities.getEntityConfig was unable to identify file '+global.routes.path('deployment', config_path+'/config.json'));
    }
