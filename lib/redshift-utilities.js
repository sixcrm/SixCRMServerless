...
class RedshiftUtilities extends AWSUtilities {
    ...
    constructor(){
      ...
      let endpoint_url = global.SixCRM.configuration.site_config.redshift.endpoint;
      let port = global.SixCRM.configuration.site_config.redshift.port;
      let endpoint;

      if (endpoint_url && port) {
        endpoint = global.SixCRM.configuration.site_config.redshift.endpoint+':'+global.SixCRM.configuration.site_config.redshift.port
      }

      this.redshift = new AWS.Redshift({
        apiVersion: '2013-01-01',
        region: global.SixCRM.configuration.site_config.redshift.region || global.SixCRM.configuration.site_config.aws.region,
        endpoint: endpoint
      });
