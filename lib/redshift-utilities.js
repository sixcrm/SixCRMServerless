      ...
      return new Promise((resolve, reject) => {
          this.redshift.createCluster(parameters, (error, data) => {
              if (error) {
                  ...
                  let host = data['something']; // Technical Debt: Replace with real redshift cluster host information.
                  let jdbc_host = 'jdbc:redshift://' + host; // Technical Debt: Update this as well.

                  return Promise.all([
                      global.SixCRM.configuration.propagateCache('all', 'redshift.host', host),
                      global.SixCRM.configuration.propagateCache('all', 'kinesis.firehose.streams.transaction_stream.RedshiftDestinationConfiguration.ClusterJDBCURL', jdbc_host),
                      global.SixCRM.configuration.propagateCache('all', 'kinesis.firehose.streams.sessions_stream.RedshiftDestinationConfiguration.ClusterJDBCURL', jdbc_host),
                      global.SixCRM.configuration.propagateCache('all', 'kinesis.firehose.streams.event_stream.RedshiftDestinationConfiguration.ClusterJDBCURL', jdbc_host),
                      global.SixCRM.configuration.propagateCache('all', 'kinesis.firehose.streams.activity_stream.RedshiftDestinationConfiguration.ClusterJDBCURL', jdbc_host)
                  ]).then(() => {
                      return data;
                  });
              }
