...
class RedshiftUtilities extends AWSUtilities {
    ...
    createParametersObject(group_name){
      ...
      let redshift_configuration = this.readRedshiftConfiguration();

      console.log(redshift_configuration.user);
      configuration_groups[group_name].forEach((key) => {

        let discovered_data = objectutilities.recurseByDepth(redshift_configuration, function(p_key){
    ...
    readRedshiftConfiguration(){

      du.debug('Read local configuration file');

      let redshift_configuration = global.SixCRM.routes.include('deployment','redshift/config/master.json');

      return redshift_configuration;
