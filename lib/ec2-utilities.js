...
const AWSUtilities = global.routes.include('lib', 'aws-utilities.js')
...
module.exports = class EC2Utilities extends AWSUtilities {
    ...
    assureSecurityGroup(parameters){
      ...
      return this.securityGroupExists(security_group_identifier).then((result) => {
        ...
        if(result !== false){
          ...
          du.info('Security group exists...');

          return result;

        }else{

          du.info('Creating security group...');
    ...
    securityGroupExists(security_group, use_cache, retry){
      ...
      if(retry !== false && retry < this.max_retry_attempts){
        ...
        du.info('Retrying ('+mathutilities.appendOrdinalSuffix(retry)+' attempt...)');
    ...
    addSecurityGroupIngressRules(parameters){
      ...
      if(!_.has(parameters, 'GroupId')){
        ...
        return new Promise((resolve) => {
          ...
          this.removeExistingIngressRules(parameters).then(() => {
            this.ec2.authorizeSecurityGroupIngress(parameters, (error, data) => resolve(this.AWSCallback(error, data)));
          });
    ...
    addSecurityGroupEgressRules(parameters){
      ...
      if(!_.has(parameters, 'GroupId')){
        ...
        return new Promise((resolve) => {
          ...
          this.removeExistingEgressRules(parameters).then(() => {

            parameters = objectutilities.subtractiveFilter(['GroupName'], parameters);

            this.ec2.authorizeSecurityGroupEgress(parameters, (error, data) => resolve(this.AWSCallback(error, data)));
    ...
    removeExistingIngressRules(parameters){
      ...
      return new Promise((resolve) => {
        ...
        this.securityGroupExists(parameters.GroupId).then((result) => {
    ...
    filterRule(rule){
      ...
      if(_.contains(['tcp','udp'], rule.IpProtocol)){
        return objectutilities.additiveFilter(['IpProtocol', 'IpRanges','FromPort','ToPort'], rule);
      }else if(rule.IpProtocol == '-1'){
        return objectutilities.additiveFilter(['IpProtocol', 'IpRanges'], rule);
      }
    ...
    revokeSecurityGroupIngress(parameters){
      ...
      return new Promise((resolve) => {
        ...
        this.ec2.revokeSecurityGroupIngress(parameters, (error, data) => resolve(this.AWSCallback(error, data)));
    ...
    destroySecurityGroup(parameters){
      ...
      return new Promise((resolve) => {
        ...
        this.ec2.deleteSecurityGroup(parameters, (error, data) => resolve(this.AWSCallback(error, data)));
