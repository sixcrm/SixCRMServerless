...
const ses = global.routes.include('lib', 'ses-utilities');
...
class EmailNotificationUtilities {
    ...
    sendNotificationViaEmail(notification_object, email_address) {
        ...
        return notificationController.isValidNotification(notification_object).then(() => {
            ...
            let email = {
                to: [email_address],
                subject: notification_object.message,
    ...
    formatEmailBody(notification_object) {
        return `You received a notification with message "${notification_object.message}". Thanks for using SixCRM!`;
    }
}
