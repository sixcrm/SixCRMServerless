...
class ArrayUtilities {
	...
	static forEach(array, iterator) {
		...
		if (!_.isFunction(iterator)) {
			eu.throwError('server', 'ArrayUtilities.forEach iterator is not a function.')
		}
	...
	static flatten(multidimensional_array, depth) {
		...
		if (depth > 20) {
			eu.throwError('server', 'Array Utilities flatten recursion depth exceeded.');
		}
	...
	static serialPromises(array_of_promises) {
		...
		return this.reduce(
			...
			.catch((ex) => {
				eu.error('Serial promise chain exception', ex);
				throw ex;
			});
