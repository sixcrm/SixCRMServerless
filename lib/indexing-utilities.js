...
class IndexingUtilities {
	...
	assureSuggestorFields(processed_document){
		...
		if(!_.has(processed_document, 'suggestion_field_1')){
			
			let suggestion_field_1 = '';
			
			if(_.has(processed_document, 'name')){
				
				suggestion_field_1 = processed_document.name;
				
			}else if(_.has(processed_document, 'firstname') || _.has(processed_document, 'lastname')){
				
				if(_.has(processed_document, 'firstname')){
					suggestion_field_1 += processed_document.firstname;
				}
				
				if(_.has(processed_document, 'lastname')){
					if(suggestion_field_1.length > 0){ suggestion_field_1 += ' '; }
					suggestion_field_1 += processed_document.lastname;
				}
				
			}else if(_.has(processed_document, 'trackingnumber')){
			
				suggestion_field_1 = processed_document.trackingnumber;
				
			}else if(_.has(processed_document, 'alias')){
			
				suggestion_field_1 = processed_document.alias;
				
			}
			
			processed_document['suggestion_field_1'] = suggestion_field_1;
	...
	createIndexingDocument(documents_array){
		...
		documents_array.forEach((document) => {
			...
			if(_.has(parsed_document, 'index_action') && _.contains(['add','delete'], parsed_document.index_action) && _.has(parsed_document, 'id')){		
				...
				processed_document = this.assureSuggestorFields(processed_document);
