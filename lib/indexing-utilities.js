...
class IndexingUtilities {
		...
		return new Promise((resolve, reject) => {
			...
			if(!_.has(entity, 'id')){
				...
				if(!validator.isUUID(entity.id, '4')){
					...
					if(_.has(entity, 'entity_type') && entity.entity_type == 'user'){
						
						du.debug('Entity ID type bypass: user');
				...
				if(_.has(entity, 'index_action') && _.contains(['add', 'delete'], entity.index_action)){
					...
					if(_.has(entity, 'entity_type')){
						...
						if(_.contains(this.indexing_entities, entity.entity_type)){
							...
							sqs.sendMessage({message_body: abridged_entity_string, queue_url: process.env.search_indexing_queue_url}, (error) => {
	...
	//Technical Debt: Add the address fields from the address objects where pertinent
