...
class IndexingUtilities {
    ...
    addToSearchIndex(entity){
        ...
        if(!_.has(entity, 'entity_type')){ return Promise.reject(eu.getError('validation','Undefined "entity_type" field on entity indexing record.')); }
    ...
    //Technical Debt:  This is inappropriately named.
    indexEntity(entity){
            ...
            this.validateEntity(entity).then((validation) => {
                ...
                if(validation == true){
                    ...
                    if(_.has(validation, 'errors')){
                        ...
                        return reject(eu.getError('validation','Entity failed validation: '+validation.errors.join(', ')));

                    }else{

                        return reject(eu.getError('validation','Entity validation returned unrecognized structure.'));
            ...
            if(!_.contains(['add', 'delete'], entity.index_action)){
                ...
                if(entity.index_action == 'add'){
                    ...
                    let has_indexable_field = this.indexing_list.some((indexable_field) => {
                        ...
                        //Technical Debt:  This seems out of place...
            ...
            if(_.has(entity, 'index_action') && _.contains(['add', 'delete'], entity.index_action)){
                ...
                if(_.has(entity, 'entity_type')){
                    ...
                    if(_.contains(this.indexing_entities, entity.entity_type)){
                        ...
                        sqs.sendMessage({message_body: abridged_entity_string, queue: 'search_indexing'}).then((response) => {
                    ...
                    return reject(eu.getError('validation','Indexable entities must have a "Entity Type".'));

                }

            }else{

                return reject(eu.getError('validation','Indexable entities must have a "index_action" which is either "add" or "delete".'));
    ...
    parseMessage(message){
        ...
        eu.throwError('validation','Unable to acquire message body.');
    ...
    assureIdInFields(document) {
        document.fields.id = document.id;
    ...
    createIndexingDocument(documents_array){
        ...
        du.debug('Is argument array?', _.isArray(documents_array));
        ...
        documents_array.forEach((document) => {
            ...
            if(_.has(parsed_document, 'index_action') && _.contains(['add','delete'], parsed_document.index_action) && _.has(parsed_document, 'id')){
                ...
                processed_document = this.assureIdInFields(processed_document);
