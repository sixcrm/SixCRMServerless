...
class S3Utilities extends AWSUtilities{
    ...
    deleteBucketObjects(bucket_name){
      ...
      return this.listObjects(bucket_name, null, {Prefix:bucket_name+'/'}).then(bucket_objects => {
      ...
      return new Promise((resolve) => {
        ...
        if(!_.isUndefined(additional_properties) && _.isObject(additional_properties)){
          ...
          for(var key in additional_properties){
            if(_.contains(['Delimiter','EncodingType','FetchOwner','StartAfter','RequestPayer'], key)){
    ...
    deleteObjects(bucket_name, bucket_objects){
      ...
      return new Promise((resolve) => {
        ...
        if(!_.isArray(bucket_objects)){
          ...
          eu.throwError('server', 'S3Utilities.deleteObjects assumes bucket_objects to be an array');

        }

        if(bucket_objects.length > this.maximum_delete_batch_size){

          eu.throwError('server', 'S3Utilities.deleteObjects bucket_objects array is too large');
        ...
        return new Promise((resolve, reject) => {
            ...
            this.bucketExists(bucket_name).then((result) => {
                ...
                if(result !== true){
                  ...
                  du.output('Bucket not found, creating');
                  ...
                  du.output('Bucket found, skipping');
    ...
    getObject(bucket, path){
      ...
      let parameters = {Bucket: bucket, Key: path};

      return this.s3.getObject(parameters, (error, data) => Promise.resolve(this.AWSCallback(error, data)));
