...
module.exports = class Timestamp {
	...
	static upcoming(dayname, week_offset = 0, at = null){
		...
		let beginning_of_day = moment().utc().day(_.indexOf(day_translation, _.toLower(dayname)) + week_offset).startOf('day');

		if(!_.isNull(at)){
			let time = moment(at, 'HH:mm a').utc();
