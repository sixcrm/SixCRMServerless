...
class Timestamp {
  ...
  static convertToFormat(date, format){
    ...
    return moment.utc(this_date).format(format);
  ...
  static getThisHourInISO8601() {
    let time = moment.utc().minute(0).second(0).millisecond(0);

    return this.convertToISO8601(time.format());
  }

  static getLastHourInISO8601() {
    let time = moment.utc().subtract(1, 'h').minute(0).second(0).millisecond(0);

    return this.convertToISO8601(time.format());
  }

  static withinLastMinute(date) {
      let difference_in_seconds = moment.utc().diff(date, 'seconds');
