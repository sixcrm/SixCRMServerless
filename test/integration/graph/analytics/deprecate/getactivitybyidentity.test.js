...
let test_query = global.SixCRM.routes.path('handlers','/endpoints/graph/queries/analytics/deprecate/get-activity-by-identifier.json');
	...
	it('Should return return a 200 HTTP response code and a correctly formatted response', function (done) {
		...
		this_request.post('graph/'+account.id, {timeout: 5000})
			...
			.end(function(err, response){
				...
				assert.isTrue(tu.validateGraphResponse(response.body, 'analytics/deprecate/activity/listactivitybyidentity'));
