...
let test_name = 'Events By Facet';
let test_query = global.SixCRM.routes.path('handlers','endpoints/graph/queries/analytics/getEventsByFacet.json');
	...
	it('Should return return a 200 HTTP response code', function (done) {
		...
		this_request.post('graph/'+account.id,{timeout: 5000})
			...
			.end(function(err, response){
				...
				assert.isTrue(tu.validateGraphResponse(response.body, 'analytics/event/eventsbyfacet'));
