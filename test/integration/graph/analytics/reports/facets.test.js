...
let test_name = 'Report facets';

let test_query = global.SixCRM.routes.include('handlers', 'endpoints/graph/queries/analytics/reports/facets/facets.json');
	...
	it('Should return return a 200 HTTP response code and a correctly formatted response', function (done) {
		...
		this_request.post('graph/' + account.id, {
			...
			.end(function (err, response) {
				...
				assert.isTrue(tu.validateGraphResponse(response.body.response.data.analyticsfacets, 'analytics/reports/analytics-facets'));
