...
class SMTPProviderController {

	constructor(){
	
	}
	
	listSMTPProviders(cursor, limit){
		...
		return new Promise((resolve, reject) => {
			...
			dynamoutilities.scanRecordsFull(process.env.smtp_providers_table, query_parameters, (error, data) => {
	...
	getSMTPProvider(id){
		...
		return new Promise((resolve, reject) => {
			...
			dynamoutilities.queryRecords(process.env.smtp_providers_table, 'id = :idv', {':idv': id}, null, (error, data) => {
				...
				if(_.isObject(data) && _.isArray(data)){
					...
					if(data.length == 1){
						...
						if(data.length > 1){
							...
							reject(new Error('Multiple SMTP providers returned where one should be returned.'));
